...
@section('multiple-action-button')
    @if((isset($permissions) && in_array('show timesheet',$permissions)) || (isset($currentWorkspace) && $currentWorkspace->permission == 'Owner'))
        <a href="{{route($client_keyword.'projects.timesheet.index',[$currentWorkspace->slug,$project->id])}}" class="btn btn-xs btn-white btn-icon-only width-auto">{{ __('Timesheet')}}</a>
    @endif
    @if((isset($permissions) && in_array('show gantt',$permissions)) || (isset($currentWorkspace) && $currentWorkspace->permission == 'Owner'))
        <a href="{{route($client_keyword.'projects.gantt',[$currentWorkspace->slug,$project->id])}}" class="btn btn-xs btn-white btn-icon-only width-auto">{{ __('Gantt Chart')}}</a>
    @endif
    @if((isset($permissions) && in_array('show task',$permissions)) || (isset($currentWorkspace) && $currentWorkspace->permission == 'Owner'))
        <a href="{{route($client_keyword.'projects.task.board',[$currentWorkspace->slug,$project->id])}}" class="btn btn-xs btn-white btn-icon-only width-auto">{{ __('Task Board')}}</a>
    @endif
{{--    @if((isset($permissions) && in_array('show bug report',$permissions)) || (isset($currentWorkspace) && $currentWorkspace->permission == 'Owner'))--}}
{{--        <a href="{{route($client_keyword.'projects.bug.report',[$currentWorkspace->slug,$project->id])}}" class="btn btn-xs btn-white btn-icon-only width-auto">{{ __('Bug Report')}}</a>--}}
{{--    @endif--}}
@endsection
                                        ...
                                        <li class="list-group-item px-0">
                                            <div class="row align-items-center">
                                                <div class="col">
                                                    ...
                                                    <div class="h6 mb-0">{{Utility::dateFormat($project->start_date)}}</div>
                                                </div>
                                                <div class="col">
                                                    <small>{{ __('Due Date') }}:</small>
                                                    <div class="h6 mb-0">{{Utility::dateFormat($project->end_date)}}</div>
                                                </div>
                                                <div class="col-auto">
