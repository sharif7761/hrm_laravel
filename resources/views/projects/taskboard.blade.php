...
@section('multiple-action-button')
    @if((isset($permissions) && in_array('create task',$permissions)) || ($currentWorkspace && $currentWorkspace->permission == 'Owner'))
        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Task') }}" data-url="{{route($client_keyword.'tasks.create',[$currentWorkspace->slug,$project->id])}}"><i class="fas fa-plus mr-1"></i>{{ __('Add New')}}</a>
    @endif
                                            ...
                                            @foreach($stage->tasks as $task)
                                                <div class="card card-progress draggable-item border shadow-none mb-3" id="{{$task->id}}">
                                                    <div class="card-body">
                                                                    ...
                                                                    @if($currentWorkspace->permission == 'Owner' || isset($permissions))
                                                                        <div class="dropdown float-right">
                                                                            ...
                                                                            <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(22px, 31px, 0px);">
                                                                                <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('View Task') }}" data-url="{{route($client_keyword.'tasks.show',[$currentWorkspace->slug,$task->project_id,$task->id])}}">
                                                                                {{__('View')}}</a>
                                                                                @if($currentWorkspace->permission == 'Owner')
                                                                                    <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Task') }}" data-url="{{route('tasks.edit',[$currentWorkspace->slug,$task->project_id,$task->id])}}">
                                                                                        {{__('Edit')}}</a>
                                                                                    <a href="#" class="dropdown-item" data-confirm="{{ __('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?') }}" data-confirm-yes="document.getElementById('delete-form-{{$task->id}}').submit();">
                                                                                        {{__('Delete')}}
                                                                                    </a>
                                                                                ...
                                                                                @elseif(isset($permissions))
                                                                                    @if(in_array('edit task',$permissions))
                                                                                        <a href="#" class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Task') }}" data-url="{{route($client_keyword.'tasks.edit',[$currentWorkspace->slug,$task->project_id,$task->id])}}">
                                                                                            {{__('Edit')}}
                                                                                        </a>
                                                                                    @endif
                                                        ...
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <a href="#" data-url="{{route($client_keyword.'tasks.show',[$currentWorkspace->slug,$task->project_id,$task->id])}}" data-ajax-popup="true" data-title="{{__('Task Detail')}}" class="h6 task-title">{{$task->title}}</a>
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col">
                                                                <div class="action-item">
                                                                    {{\App\Utility::dateFormat($task->start_date)}}
                                                                </div>
                                                            </div>
                                                            <div class="col text-right">
                                                                <div class="action-item">
                                                                    {{\App\Utility::dateFormat($task->due_date)}}
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row">
