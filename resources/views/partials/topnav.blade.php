                ...
                <li class="nav-item dropdown dropdown-animate">
                    @if(isset($currentWorkspace) && $currentWorkspace)
                        @auth('web')
                            ...
                            <a class="nav-link nav-link-icon @if(count($notifications))beep @endif" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-bell"></i></a>
                    ...
                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right dropdown-menu-arrow">
                        ...
                        @foreach(Auth::user()->workspace as $workspace)
                            @if($workspace->is_active)
                                <a href="@if(isset($currentWorkspace) && $currentWorkspace->id == $workspace->id)#@else @auth('web'){{ route('change-workspace',$workspace->id) }}@elseauth{{ route('client.change-workspace',$workspace->id) }}@endauth @endif" title="{{ $workspace->name }}" class="dropdown-item">
                        ...
                        @if(isset($currentWorkspace) && $currentWorkspace)
                            @auth('web')
                                @if(Auth::user()->id == $currentWorkspace->created_by )
                                    <a href="#" class="dropdown-item" data-confirm="{{ __('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?') }}" data-confirm-yes="document.getElementById('remove-workspace-form').submit();">
                                        <i class="fa fa-trash"></i>
                                        <span>{{ __('Remove Me From This Workspace')}}</span>
                                    </a>
                                    <form id="remove-workspace-form" action="{{ route('delete-workspace', ['id' => $currentWorkspace->id]) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                @else
                                    ...
                                    <form id="remove-workspace-form" action="{{ route('leave-workspace', ['id' => $currentWorkspace->id]) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                @endif
                        ...
                        <a href="@auth('web'){{ route('users.my.account') }}@elseauth{{ route('client.users.my.account') }}@endauth" class="dropdown-item">
            ...
            <ul class="navbar-nav align-items-center d-none d-lg-flex">
                ...
                <li class="nav-item dropdown dropdown-animate">
                    ...
                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-left dropdown-menu-arrow">
                        ...
                        @foreach(Auth::user()->workspace as $workspace)
                            @if($workspace->is_active)
                                <a href="@if($currentWorkspace->id == $workspace->id)#@else @auth('web'){{ route('change-workspace',$workspace->id) }}@elseauth{{ route('client.change-workspace',$workspace->id) }}@endauth @endif" title="{{ $workspace->name }}" class="dropdown-item">
                                    @if($currentWorkspace->id == $workspace->id)
                                        <i class="fa fa-check"></i>
                                    @endif
                                    <span>{{ $workspace->name }}</span>
                        ...
                        <form id="logout-form1" action="@auth('web'){{ route('logout') }}@elseauth{{ route('client.logout') }}@endauth" method="POST" style="display: none;">
                            @csrf
                        </form>
                    </div>
                </li>
...
{{--                        <li class="nav-item dropdown dropdown-animate">--}}
{{--                            <a class="nav-link nav-link-icon message-toggle @if(count($messages)) beep @endif" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-comment-alt"></i></a>--}}
{{--                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg dropdown-menu-arrow p-0">--}}
{{--                                <div class="py-3 px-3">--}}
...
{{--                                </div>--}}
{{--                                <div class="py-3 text-center">--}}
{{--                                    <a href="{{route('chats')}}" class="link link-sm link--style-3">{{__('View All')}} <i class="fa fa-chevron-right"></i></a>--}}
{{--                                </div>--}}
{{--                            </div>--}}
{{--                        </li>--}}
{{--                        <li class="nav-item dropdown dropdown-animate">--}}
...
{{--                            <a class="nav-link nav-link-icon notification-toggle @if(count($notifications)) beep @endif" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-bell"></i></a>--}}
...
{{--                @endif--}}
                ...
                <li class="nav-item dropdown dropdown-list-toggle">
                    <a href="#" data-toggle="dropdown" class="nav-link nav-link-icon">
                        <span class="align-middle"><i class="fas fa-globe-europe mr-2"></i>{{Str::upper($currantLang)}}</span>
                    </a>
...
{{--                                </a>--}}
{{--                            @endforeach--}}
{{--                        @elseif(isset($currentWorkspace) && $currentWorkspace && $currentWorkspace->permission == 'Owner')--}}
