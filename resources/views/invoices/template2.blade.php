                            ...
                            <div data-v-37eeda86="" class="d-inner">
                                <div data-v-37eeda86="" class="row">
                                    <div data-v-37eeda86="" class="col-2">
                                        <img
                                            src="@if($currentWorkspace->logo){{asset(Storage::url('logo/'.$currentWorkspace->logo))}}@else{{asset(Storage::url('logo/logo-blue.png'))}}@endif"
                                            style="max-width: 150px"/>
                                    </div>
                                    <div data-v-37eeda86="" class="col-2 text-right">
                                        <p>@if($currentWorkspace->company){{$currentWorkspace->company}}@endif</p>
                                        <p>
                                            @if($currentWorkspace->address){{$currentWorkspace->address}}@endif
                                            @if($currentWorkspace->city) <br> {{$currentWorkspace->city}}@endif
                                            @if($currentWorkspace->state), {{$currentWorkspace->state}}@endif
                                            @if($currentWorkspace->zipcode)- {{$currentWorkspace->zipcode}}@endif
                                ...
                                <div data-v-37eeda86="" class="row">
                                    <div data-v-37eeda86="" class="col-66">
                                        <strong class="tu mb5" style="color: {{($color == '#ffffff') ? 'black': $color}};">{{__('Billing Address')}}:</strong>
                                        @if($invoice->client)
                                            <p>{{$invoice->client->name}}</p>
                                            @if($invoice->client->address){{$invoice->client->address}}@endif
                                            @if($invoice->client->city) <br> {{$invoice->client->city}}@endif
                                    ...
                                    <div data-v-37eeda86="" class="col-33"><strong data-v-37eeda86="" class="tu mb5"
                                        ...
                                        <table data-v-37eeda86="" class="summary-table">
                                            ...
                                            <tr data-v-37eeda86="">
                                                ...
                                                <td data-v-37eeda86=""
                                                    class="text-right">{{Utility::invoiceNumberFormat($invoice->invoice_id)}}</td>
                                            </tr>
                                            <tr data-v-37eeda86="">
                                                <td data-v-37eeda86="" class="tu">{{__('Project')}}:</td>
                                                <td data-v-37eeda86=""
                                                    class="text-right">{{$invoice->project->name}}</td>
                                            </tr>
                                            <tr data-v-37eeda86="">
                                                <td data-v-37eeda86="" class="tu">{{__('Issue Date')}}:</td>
                                                <td data-v-37eeda86=""
                                                    class="text-right">{{Utility::dateFormat($invoice->issue_date)}}</td>
                                            </tr>
                                            <tr data-v-37eeda86="">
                                                <td data-v-37eeda86="" class="tu">{{__('Due Date')}}:</td>
                                                <td data-v-37eeda86=""
                                                    class="text-right">{{Utility::dateFormat($invoice->due_date)}}</td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                <div data-v-37eeda86="" class="d-table">
                                    <div data-v-37eeda86="" class="d-table">
                                        <div data-v-37eeda86="" class="tu d-table-tr"
                                             style="color: {{($color == '#ffffff') ? 'black': $color}}; border-bottom: 1px solid {{$color}}; border-top: 1px solid {{$color}};">
                                            <div class="d-table-th w-2">{{__('#')}}</div>
                                            <div class="d-table-th w-17">{{__('Item')}}</div>
                                            <div class="d-table-th w-5 text-right">{{__('Totals')}}</div>
                                        </div>
                                                ...
                                                @foreach($items as $key => $item)
                                                    <div class="d-table-tr"
                                                        ...
                                                        <div class="d-table-td w-17">
                                                            <pre data-v-f2a183a6="">{{$item->task->title}}</pre>
                                                        </div>
                                                        <div class="d-table-td w-5 text-right">
                                                            <span>{{$currentWorkspace->priceFormat($item->price*$item->qty)}}</span>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            @endif
                                        ...
                                        <div data-v-37eeda86="" class="d-table-footer">
                                            ...
                                            <div class="d-table-summary">
                                                <div class="d-table-summary-item">
                                                    ...
                                                    <div
                                                        class="d-table-value">{{$currentWorkspace->priceFormat($invoice->getSubTotal())}}</div>
                                                </div>
                                                @if($invoice->discount)
                                                    <div class="d-table-summary-item">
                                                        <div class="tu d-table-label">{{__('Discount')}}:</div>
                                                        <div
                                                            class="d-table-value">- {{$currentWorkspace->priceFormat($invoice->discount)}}</div>
                                                    </div>
                                                @endif
                                                @if($invoice->tax)
                                                    <div class="d-table-summary-item">
                                                        <div class="tu d-table-label">{{$invoice->tax->name}}
                                                            ({{$invoice->tax->rate}}%):
                                                        </div>
                                                        <div
                                                            class="d-table-value">{{$currentWorkspace->priceFormat($invoice->getTaxTotal())}}</div>
                                                    </div>
                                                @endif
                                                <div class="d-table-summary-item"
                                                     style="border-top: 1px solid {{$color}}; border-bottom: 1px solid {{$color}};">
                                                    <div class="tu d-table-label">{{__('Total')}}:</div>
                                                    <div class="d-table-value">{{$currentWorkspace->priceFormat($invoice->getTotal())}}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
